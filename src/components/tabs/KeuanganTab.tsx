...
import { TagihanItem, StudentProfile, TabunganSantri, RiwayatTabunganItem } from '../../types';
import { playTapSound, playSuccessSound } from '../../utils/audio';
import { subscribeTabunganFromFirestore } from '../../services/firestoreService';
import { TABUNGAN_SANTRI_LIST } from '../../data/madrasahCompleteData';
...
export const KeuanganTab: React.FC<KeuanganTabProps> = ({
  ...
  const [activeMenuSegment, setActiveMenuSegment] = useState<'tagihan' | 'tabungan'>('tagihan');
  ...
  // Tabungan State for Student
  const [tabunganList, setTabunganList] = useState<TabunganSantri[]>(TABUNGAN_SANTRI_LIST);
  const [isOpenBukuTabungan, setIsOpenBukuTabungan] = useState(false);
  const [selectedKuitansi, setSelectedKuitansi] = useState<{ santri: TabunganSantri; transaksi: RiwayatTabunganItem } | null>(null);

  useEffect(() => {
    const unsub = subscribeTabunganFromFirestore((list) => {
    ...
    <div className="flex flex-col gap-3 p-4 pb-8 bg-slate-50">
      ...
      <div className="grid grid-cols-2 bg-white p-1 rounded-2xl border border-slate-200 shadow-xs">
        ...
        <button
          onClick={() => {
            playTapSound();
            setActiveMenuSegment('tabungan');
          }}
          className={`py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 transition-all ${
            activeMenuSegment === 'tabungan'
              ? 'bg-teal-700 text-white shadow-xs font-black'
              : 'text-slate-600 hover:text-slate-900'
          }`}
        >
          <Wallet className="w-3.5 h-3.5" />
          <span>Buku Tabungan Saya</span>
        </button>
      </div>

      {/* ================= VIEW 1: TAGIHAN SYAHRIYAH & SPP ================= */}
      {activeMenuSegment === 'tagihan' && (
        <>
      ...
      {/* ================= VIEW 2: BUKU TABUNGAN SANTRI SAYA ================= */}
      {activeMenuSegment === 'tabungan' && studentTabungan && (
        <div className="space-y-3 animate-in fade-in">
          {/* Kartu Saldo Tabungan Santri */}
          <div className="rounded-3xl bg-gradient-to-br from-teal-700 via-emerald-800 to-teal-900 p-4 border border-teal-500/40 shadow-md text-white relative overflow-hidden">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-teal-100 font-medium">Buku Simpanan Santri Digital</span>
              <span className="text-[10px] font-bold bg-amber-400 text-emerald-950 px-2.5 py-0.5 rounded-full font-mono">
                {studentTabungan.noRekening || studentTabungan.noInduk}
              </span>
            </div>

            <div className="text-2xl font-extrabold font-mono tracking-tight text-amber-300 mb-1">
              {formatRupiah(studentTabungan.jumlahTabungan)}
            </div>
            <p className="text-xs text-teal-100">
              Saldo tabungan aman & tercatat di Koperasi Madrasah (KOPAS)
            </p>

            <div className="grid grid-cols-2 gap-2 mt-4 pt-3 border-t border-white/20 text-xs">
              <div>
                <span className="text-[10px] text-teal-200 font-medium">Program:</span>
                <div className="font-bold text-white truncate">{studentTabungan.programTabungan || 'Reguler/Saku'}</div>
              </div>
              <div className="text-right">
                <span className="text-[10px] text-teal-200 font-medium">Terakhir Transaksi:</span>
                <div className="font-mono font-bold text-teal-100">{studentTabungan.terakhirTransaksi || '-'}</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                playTapSound();
                setIsOpenBukuTabungan(true);
              }}
              className="flex-1 py-2.5 px-3 bg-teal-700 hover:bg-teal-800 text-white rounded-2xl text-xs font-bold flex items-center justify-center gap-1.5 shadow-2xs transition-colors"
            >
              <BookOpen className="w-4 h-4" />
              <span>Buka Buku Rekening Lengkap</span>
            </button>
          </div>

          {/* Mutasi Terakhir */}
          <div className="bg-white rounded-3xl border border-slate-200 p-3.5 shadow-xs space-y-2.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-extrabold text-slate-800 flex items-center gap-1.5">
                <Calendar className="w-3.5 h-3.5 text-teal-600" />
                Mutasi Transaksi Tabungan ({studentTabungan.riwayat.length})
              </span>
              <button
                onClick={() => setIsOpenBukuTabungan(true)}
            ...
            {studentTabungan.riwayat.length === 0 ? (
              <div className="p-6 text-center text-slate-400 text-xs">
                Belum ada catatan mutasi transaksi tabungan.
              </div>
            ) : (
              <div className="space-y-1.5">
                {studentTabungan.riwayat.slice(0, 5).map((rw) => (
                  <div
                    key={rw.id}
                    className="p-2.5 bg-slate-50 hover:bg-slate-100/80 rounded-2xl border border-slate-200 flex items-center justify-between text-xs transition-colors"
                  >
                    <div className="flex items-center gap-2">
                      <div className={`w-7 h-7 rounded-xl flex items-center justify-center shrink-0 ${
                        rw.jenis === 'Setor' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'
                      }`}>
                        {rw.jenis === 'Setor' ? <ArrowDownLeft className="w-3.5 h-3.5" /> : <ArrowUpRight className="w-3.5 h-3.5" />}
                      </div>
                      <div>
                        <span className="font-extrabold text-slate-800 block leading-tight">{rw.keterangan}</span>
                        <span className="text-[10px] text-slate-400 font-mono">{rw.tanggal} {rw.waktu ? `• ${rw.waktu}` : ''}</span>
                      </div>
                    </div>
                    ...
                    <div className="text-right flex items-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          playTapSound();
                          setSelectedKuitansi({ santri: studentTabungan, transaksi: rw });
                        }}
                        className="p-1 rounded-lg bg-white border border-slate-200 text-slate-600 hover:text-emerald-700"
      ...
      {/* Modal Buku Tabungan Digital */}
      {isOpenBukuTabungan && studentTabungan && (
        <BukuTabunganModal
          santri={studentTabungan}
          onClose={() => setIsOpenBukuTabungan(false)}
          canEdit={false}
          onViewKuitansi={(s, t) => {
            setSelectedKuitansi({ santri: s, transaksi: t });
          }}
        />
      )}

      {/* Modal Kuitansi Mutasi */}
      {selectedKuitansi && (
        <KuitansiTabunganModal
          santri={selectedKuitansi.santri}
          transaksi={selectedKuitansi.transaksi}
